...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $nip = $_POST['username']; // Assuming you are using NIP as the username
    $password = $_POST['password'];

    // Debug: Echo the input values
    echo "NIP: " . htmlspecialchars($nip) . "<br>";
    echo "Password: " . htmlspecialchars($password) . "<br>";

    // Prepare and execute the query
    $stmt = $con->prepare("SELECT * FROM tb_kepsek WHERE nip=? AND status='Y'");
    $stmt->bind_param("s", $nip);
    $stmt->execute();
    $result = $stmt->get_result();
    $jml = $result->num_rows;

    // Debug: Echo the number of rows found
    echo "Number of rows found: " . $jml . "<br>";

    $d = $result->fetch_assoc();

    // Debug: Echo the fetched result
    echo "Fetched result: " . print_r($d, true) . "<br>";

    // Check if a user was found and the password matches
    if ($jml > 0 && password_verify($password, $d['password'])) {
        $_SESSION['admin'] = $d['id_kepsek'];
                    ...
                    <div class="container">
                        <div class="page-inner py-5">
                            <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
                                <div>
                                    <h2 class="text-white pb-2 fw-bold">SMP NEGERI 1 MUHAMMADIYAH</h2>
                                    <!-- <h5 class="text-white op-7 mb-2">Absen Lebih mudah gunakan Aplikasi Absensi</h5> -->
                                </div>
                                <div class="ml-md-auto py-2 py-md-0">
                                    <a href="#" class="btn btn-white btn-border btn-round mr-2">Aplikasi Presensi Siswa</a>
                                ...
                                <div class="card-body">
                                    <form action="" method="post">
                                        <div class="form-group form-floating-label">
                                            ...
                                            <label class="placeholder">NIP</label>
                                        </div>
                                        <div class="form-group form-floating-label">
