...
if ($d = mysqli_fetch_array($kelasMengajar)) { 
?>
...
<div class="page-inner">
	...
	<div class="row">
		<div class="col-md-12 col-xs-12 mt-3">
			...
			while ($bulan = mysqli_fetch_array($qry)) { 
				$month = date('m', strtotime($bulan['tgl_absen']));
			?>

			<a target="_blank" href="modul/rekap/rekap_bulan.php?pelajaran=<?= $_GET['pelajaran'] ?>&bulan=<?= $month; ?>&kelas=<?= $d['id_mkelas'] ?>" style="text-decoration: none;" class="text-primary">
				<div class="alert alert-primary alert-dismissible" role="alert">
					<strong>REKAP BULAN (<?= strtoupper(namaBulan($month)) ?> <?= strtoupper(date('Y')) ?>) </strong> 
				</div>
			</a>
